...
from app.database.models import Base  # noqa: F401 - imports trigger model registration
...
# Override alembic.ini DB URL with value from .env
...
def revision_id_generator(context, revision, directives):
    """Generate sequential revision IDs like 0001, 0002, 0003, etc."""
    ...
    if os.path.exists(versions_dir):
        for filename in os.listdir(versions_dir):
            if filename.endswith(".py") and not filename.startswith("__"):
                # Extract the numeric prefix (e.g., "0001" from "0001_my_message.py")
    ...
    # Set the revision ID in the migration script
